...
class Colaboraty_transport_dinamic_st():
    ...
    def interactive_model(self):
        ...
        model_file = requests.get('https://raw.githubusercontent.com/raaraya1/transport-models-web/main/Descargables/Colaboratory_Transport.txt')
        ...
        if cost_matrix is not None and type(cap) != np.str and type(t) != np.str:
            ...
            **Nota:** Es necesario establecer, tanto en la matriz de costo como
            ...
            if coordenadas is not None:
                ...
                period_select = st.selectbox('Periodo', [str(i) for i in T])

                def rutas_from_period(periodo):
                    rutas_selec = []
                    for i in X:
                      if X[i] > 0 and i[2] == int(periodo):
                          if i[0] == 'c':
                              rutas_selec.append((last_term, i[1]))
                          elif i[1] == 'c':
                              rutas_selec.append((i[0], last_term))
                          else:
                              rutas_selec.append((i[0], i[1]))
                ...
                rutas_from_period(period_select)
